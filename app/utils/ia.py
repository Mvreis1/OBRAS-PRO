...
"""
from app.models import db, Lancamento, Obra
...
def get_contexto_empresa(empresa_id):
    ...
    totais = db.session.query(
        func.sum(func.case((Lancamento.tipo == 'Receita', Lancamento.valor), else_=0)).label('receitas'),
        func.sum(func.case((Lancamento.tipo == 'Despesa', Lancamento.valor), else_=0)).label('despesas')
    ...
    obra_maior_gasto_data = db.session.query(
        Obra.id,
        Obra.nome,
        func.sum(Lancamento.valor).label('total_gasto')
    ).join(Lancamento).filter(
        Lancamento.empresa_id == empresa_id,
        Lancamento.tipo == 'Despesa'
    ).group_by(Obra.id, Obra.nome).order_by(func.sum(Lancamento.valor).desc()).first()
    
    obra_maior_gasto = None
    if obra_maior_gasto_data:
        obra_maior_gasto = type('obj', (object,), {
...
class RespostaIA:
    ...
    @staticmethod
    def gerar_saudacao():
        return ("Olá! Sou o assistente de gestão de obras. "
                "Posso ajudar com:\n"
    ...
    def analisar_negativo(contexto, mensagem):
        ...
        if contexto['saldo_atual'] < 0:
            ...
            return "✅ Seu saldo está POSITIVO! Não há motivo para preocupação.\n\n" \
    ...
    def gerar_saldo(contexto):
        ...
        if contexto['saldo_atual'] >= 0:
            return (f"💰 Seu saldo atual é R$ {contexto['saldo_atual']:,.2f}. "
                    f"\n\nTotal de receitas: R$ {contexto['total_receitas']:,.2f}\n"
                    f"Total de despesas: R$ {contexto['total_despesas']:,.2f}")
        else:
            return (f"⚠️ ATENÇÃO: Seu saldo é NEGATIVO: R$ {contexto['saldo_atual']:,.2f}. "
                    f"\n\nTotal de receitas: R$ {contexto['total_receitas']:,.2f}\n"
                    f"Total de despesas: R$ {contexto['total_despesas']:,.2f}\n\n"
    ...
    def gerar_default():
        ...
        return ("🤔 Não entendi exatamente. Posso ajudar com:\n\n"
...
def gerar_resposta(mensagem, contexto):
    ...
    handlers = {
        ...
        'obrigado': lambda: RespostaIA.gerar_obrigado(),
    }
