...
class PDFExport:
    ...
    def __init__(self, title='OBRAS PRO'):
        ...
        self.doc = SimpleDocTemplate(
            self.buffer, pagesize=A4, rightMargin=50, leftMargin=50, topMargin=50, bottomMargin=50
        )
        self.elements = []
    ...
    def add_footer(self):
        ...
        self.elements.append(
            Paragraph(
                f'Relatório gerado em {datetime.now().strftime("%d/%m/%Y às %H:%M")}', footer_style
            )
        )
...
def exportar_obra_pdf(obra, lancamentos, total_despesas, total_receitas, saldo):
    ...
    dados_obra = [
        ...
        [
            'Data Início:',
            obra.data_inicio.strftime('%d/%m/%Y') if obra.data_inicio else 'Não definida',
        ],
        [
            'Previsão Término:',
            obra.data_fim_prevista.strftime('%d/%m/%Y')
    ...
    dados_financeiro = [
        ...
        [
            '% Orçamento Utilizado',
            f'{(total_despesas / obra.orcamento_previsto * 100):.1f}%'
