...
from app.models import db, Notificacao, Lancamento
...
def gerar_alertas(empresa_id):
    ...
    obras_data = db.session.query(
        ...
        Lancamento.obra_id.isnot(None)
    ).group_by(Lancamento.obra_id).all()
    
    # Criar dicionário de totais por obra
    totais_obra = {od[0]: {'despesas': float(od[1] or 0), 'receitas': float(od[2] or 0)} for od in obras_data}
    ...
    lancamentos_pendentes = Lancamento.query.filter_by(
        empresa_id=empresa_id,
        status_pagamento='Pendente'
    ).filter(Lancamento.data < hoje).all()
...
def enviar_alertas_email(empresa_id):
    ...
    config = ConfigEmail.query.filter_by(
        empresa_id=empresa_id,
        alertas_ativos=True
    ).first()
