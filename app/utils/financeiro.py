...
def calcular_despesas_por_categoria(empresa_id, obra_id=None):
    ...
    query = db.session.query(
        Lancamento.categoria, func.sum(Lancamento.valor).label('total')
...
def get_obras_com_maior_gasto(empresa_id, limite=5):
    ...
    return (
        ...
        .join(subquery, Obra.id == subquery.c.obra_id)
...
def get_obras_por_status(empresa_id):
    ...
    return (
        db.session.query(Obra.status, func.count(Obra.id).label('qtd'))
