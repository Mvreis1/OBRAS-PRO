    ...
    def to_dict(self, include_totals=True):
        if not include_totals:
            return {
                ...
                'data_fim_prevista': self.data_fim_prevista.strftime('%Y-%m-%d')
                if self.data_fim_prevista
                else None,
                'data_fim_real': self.data_fim_real.strftime('%Y-%m-%d')
        ...
        result = (
            ...
            .filter(Lancamento.obra_id == self.id)
            .first()
        )
...
class ConfigIA(db.Model):
    """Configurações de IA da empresa"""

    __tablename__ = 'config_ia'

    id = db.Column(db.Integer, primary_key=True)
    empresa_id = db.Column(
        db.Integer, db.ForeignKey('empresas.id'), nullable=False, index=True, unique=True
    )
    ...
    def to_dict(self):
        return {
            ...
            'ia_padrao': self.ia_padrao,
            'ativo': self.ativo,
        }

    def get_openai_key(self):
        """Retorna a chave da OpenAI ou None"""
        return self.openai_api_key if self.openai_api_key else None

    def get_gemini_key(self):
        """Retorna a chave do Gemini ou None"""
        return self.gemini_api_key if self.gemini_api_key else None

    def get_claude_key(self):
        """Retorna a chave do Claude ou None"""
        return self.claude_api_key if self.claude_api_key else None
