...
def estatisticas():
    ...
    por_acao = db.session.query(
        LogAtividade.acao,
        func.count(LogAtividade.id)
    ).filter(
        LogAtividade.empresa_id == empresa_id,
        LogAtividade.created_at >= data_inicio
    ).group_by(LogAtividade.acao).all()
    
    # Ações por entidade
    por_entidade = db.session.query(
        LogAtividade.entidade,
        func.count(LogAtividade.id)
    ).filter(
        LogAtividade.empresa_id == empresa_id,
        LogAtividade.created_at >= data_inicio
    ).group_by(LogAtividade.entidade).all()
    
    return jsonify({
        'por_acao': {acao: qtd for acao, qtd in por_acao},
        'por_entidade': {ent: qtd for ent, qtd in por_entidade},
