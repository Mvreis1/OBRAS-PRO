...
def fornecedores():
    ...
    categorias = (
        db.session.query(Fornecedor.categoria, db.func.count(Fornecedor.id))
        .filter(
            Fornecedor.empresa_id == empresa_id,
            Fornecedor.ativo == True,  # noqa: E712
        )
        .group_by(Fornecedor.categoria)
        .all()
    )

    filter_args = {}
    if categoria:
        filter_args['categoria'] = categoria
    if busca:
...
def fornecedor_detalhe(fornecedor_id):
    ...
    paginacao = Paginacao(
        CompraFornecedor.query.filter_by(fornecedor_id=fornecedor_id).order_by(
            CompraFornecedor.data.desc()
        ),
        page=page,
        per_page=per_page,
    )
    ...
    total_compras = (
        db.session.query(db.func.sum(CompraFornecedor.valor))
        .filter(CompraFornecedor.fornecedor_id == fornecedor_id)
        .scalar()
        or 0
    )
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                ...
                return redirect(
                    url_for('fornecedores.fornecedor_detalhe', fornecedor_id=fornecedor_id)
                )
            except Exception as e:
                db.session.rollback()
                flash(f'Erro ao registrar compra: {e!s}', 'danger')

        return render_template(
            'main/compra_form.html', fornecedor=fornecedor, compra=None, obras=obras
        )
    except Exception as e:
        import traceback
...
@fornecedores_bp.route(
    '/fornecedor/<int:fornecedor_id>/compra/<int:compra_id>/pagar', methods=['POST']
)
@login_required
def pagar_compra(fornecedor_id, compra_id):
    ...
    compra = CompraFornecedor.query.filter_by(
        id=compra_id, fornecedor_id=fornecedor_id, empresa_id=empresa_id
    ).get_or_404()
...
def api_fornecedores():
    ...
    empresa_id = session.get('empresa_id')
    fornecedores = (
        Fornecedor.query.filter_by(empresa_id=empresa_id, ativo=True)
...
def api_fornecedor(fornecedor_id):
    ...
    empresa_id = session.get('empresa_id')
    fornecedor = Fornecedor.query.filter_by(id=fornecedor_id, empresa_id=empresa_id).get_or_404()
