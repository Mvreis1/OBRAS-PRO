...
def bancos():
    ...
    paginacao = Paginacao(
        ContaBancaria.query.filter_by(empresa_id=empresa_id, ativo=True).order_by(
            ContaBancaria.nome
        ),
        page=page,
        per_page=per_page,
...
def banco_detalhe(conta_id):
    ...
    paginacao = Paginacao(
        LancamentoConta.query.filter_by(conta_id=conta_id, empresa_id=empresa_id).order_by(
            LancamentoConta.data.desc()
        ),
        page=page,
        per_page=per_page,
    )
    ...
    result = (
        db.session.query(
            func.sum(
                case((LancamentoConta.tipo == 'Credito', LancamentoConta.valor), else_=0)
            ).label('credito'),
            func.sum(
                case((LancamentoConta.tipo == 'Debito', LancamentoConta.valor), else_=0)
            ).label('debito'),
        )
...
def novo_lancamento_conta(conta_id):
    ...
    return render_template(
        'banco/lancamento_form.html', conta=conta, lancamento=None, data=datetime.now()
    )
...
def api_extrato(conta_id):
    ...
    if data_inicio:
        query = query.filter(
            LancamentoConta.data >= datetime.strptime(data_inicio, '%Y-%m-%d').date()
        )
    if data_fim:
    ...
    return jsonify(
        {
            'conta': ContaBancaria.query.filter_by(id=conta_id, empresa_id=empresa_id)
            .first()
            .to_dict(),
